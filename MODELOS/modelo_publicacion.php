...
class modelo_publicacion
{
    ...
    public function mostrar_comunidad(): void
    {
        ...
        while ($fila = $resultado->fetch_assoc()) {
            ...
            $cuerpo = strip_tags($fila['cuerpo'], '<b><i><u><strong><em><p><br><ul><li><ol><h1><h2><h3><h4><h5><h6>');
            $fecha  = $fila['fecha'];
            ...
            if ($res_com->num_rows === 0) {
                echo "<p class='small ms-2 empty-comments' style='color:#94a3b8'>No hay comentarios aún.</p>";
            } else {
                while ($com = $res_com->fetch_assoc()) {
                    ...
                    <div class='comment-item d-flex mb-3'>
                        ...
                        <div class='comment-content p-2 px-3 rounded-3 w-100'
                             style='background:rgba(15,23,42,0.75);border:1px solid rgba(255,255,255,0.07)'>
                            <div class='d-flex justify-content-between align-items-baseline'>
                                <span class='fw-bold text-gold small'>{$c_autor}</span>
                                <span class='small' style='font-size:0.7rem;color:#94a3b8'>{$c_fecha}</span>
                            </div>
                            <div class='small mt-1' style='color:#e2e8f0'>{$c_cuerpo}</div>
                        </div>
                    </div>";
                }
            }
            ...
            if (isset($_SESSION['nick'])) {
                ...
                echo "<p class='text-muted small mt-2'><a href='/VISTAS/login.php' class='text-gold'>Inicia sesión</a> para comentar.</p>";
            }
