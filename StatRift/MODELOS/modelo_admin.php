...
class modelo_admin
{
    ...
    {
        // Encontrar el max id para no chocar (la bbdd no tiene auto_increment a veces en estos scripts legacy)
        $res = $this->_db->query("SELECT MAX(id) as max_id FROM campeon");
        $new_id = ($res->fetch_assoc()['max_id'] ?? 0) + 1;

        $stmt = $this->_db->prepare("INSERT INTO campeon (id, nombre, q, w, e, r, foto) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("issssss", $new_id, $nombre, $q, $w, $e, $r, $foto);
        return $stmt->execute();
    }
}
?>
